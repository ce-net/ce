...
| **App messaging** — directed app-to-app RPC over the mesh | The keystone for building control systems: apps need to send commands + receive telemetry between nodes, not just CE-internal RPCs. **Design + status: `docs/app-messaging.md`.** Stage 1 ✅: `RpcRequest::AppMessage` → `AppAck` over `/ce/rpc/1` (sender Noise-authenticated), `POST /mesh/send`, `GET /mesh/messages[/stream]` (snapshot + SSE), `ce-rs` `send_message`/`messages`. Authentication is CE, authorization is the app. Next: app pub/sub (Stage 2), sync request/response helper (Stage 3). | [CE] | Eng |
