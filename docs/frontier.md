...
| **Data layer** — content-addressed, chunked, paid P2P transfer | `sync` is one-file HTTP today. Datasets, weights, inputs, and results need BitTorrent/IPFS-shaped paid distribution. **Design: `docs/data-layer.md`** — chunked manifests over the `/blobs` store, Kademlia provider records, a `FetchChunk` RPC, per-chunk payment-channel receipts, trustless via content-addressing. Staged 1–4; durable-storage-with-proofs is the separate storage-market item that builds on top. | [CE]+[app] | Hard |
