...
| **Data layer** — content-addressed, chunked, paid P2P transfer | `sync` is one-file HTTP today. Datasets, weights, inputs, and results need BitTorrent/IPFS-shaped paid distribution. **Design + status: `docs/data-layer.md`.** Stages 1–2 ✅: chunked manifests over the `/blobs` store (`ce-rs` `put_object`/`get_object`), plus a `FetchChunk` mesh RPC + Kademlia provider records so a node pulls chunks it lacks (`GET /blobs/:hash` falls back to discover → fetch → verify → cache). Next: Stage 3 paid serving (per-chunk channel receipts) + Stage 4 `Workload` I/O by CID. Durable-storage-with-proofs is the separate storage-market item on top. | [CE]+[app] | Hard |
